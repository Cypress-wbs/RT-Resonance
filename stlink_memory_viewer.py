...
class MainWindow(QMainWindow):
    def __init__(self, map_path, symbol_addresses, target_override):
        ...
        # 数据缓冲区
        self.max_points = 500
        self.gyro_max_points = 45   # 【修改点1】：减小陀螺仪窗口长度到45个点，使高频揉弦波形完美展开、滚动更平滑快速
        self.gyro_raw_buffer = np.zeros(self.gyro_max_points)
        self.gyro_lpf_buffer = np.zeros(self.gyro_max_points)
        self.fsr1_lpf_buffer = np.zeros(self.max_points)
        self.fsr1_tension_buffer = np.zeros(self.max_points)
        self.fsr2_lpf_buffer = np.zeros(self.max_points)
        self.fsr2_tension_buffer = np.zeros(self.max_points)
        ...
        self.fsr1_plot = self.create_plot_widget("FSR 1: 食指侧发力监控", "Percent (%)")
        self.fsr1_lpf_curve = self.fsr1_plot.plot(pen=pg.mkPen("#444444", width=2, style=pg.QtCore.Qt.DotLine), name="Raw Avg")
        self.fsr1_tension_curve = self.fsr1_plot.plot(pen=pg.mkPen("#FF007F", width=2), name="Tension %")
        
        self.fsr2_plot = self.create_plot_widget("FSR 2: 拇指侧发力监控", "Percent (%)")
        self.fsr2_lpf_curve = self.fsr2_plot.plot(pen=pg.mkPen("#444444", width=2, style=pg.QtCore.Qt.DotLine), name="Raw Avg")
        self.fsr2_tension_curve = self.fsr2_plot.plot(pen=pg.mkPen("#00E5FF", width=2), name="Tension %")
        
        left_layout.addWidget(self.fsr1_plot)
        left_layout.addWidget(self.fsr2_plot)
        ...
        right_layout.addWidget(self.gyro_plot, stretch=2) # 图表占 2 份高度
        ...
        self.lbl_f1_info = QLabel("指腹发力: 0% (方差: 0)")
        self.lbl_f2_info = QLabel("虎口发力: 0% (方差: 0)")
        ...
        right_layout.addWidget(self.ai_panel, stretch=1) # 面板占 1 份高度
    ...
    def update_buffer(self, sample):
        ...
        self.fsr1_lpf_buffer[:-1] = self.fsr1_lpf_buffer[1:]
        self.fsr1_lpf_buffer[-1] = sample["g_fsr1_filtered"]
        self.fsr1_tension_buffer[:-1] = self.fsr1_tension_buffer[1:]
        self.fsr1_tension_buffer[-1] = sample["g_fsr1_tension_pct"]

        self.fsr2_lpf_buffer[:-1] = self.fsr2_lpf_buffer[1:]
        self.fsr2_lpf_buffer[-1] = sample["g_fsr2_filtered"]
        self.fsr2_tension_buffer[:-1] = self.fsr2_tension_buffer[1:]
        self.fsr2_tension_buffer[-1] = sample["g_fsr2_tension_pct"]

        # ==================== [边缘 AI 逻辑裁决引擎] ====================
        calib_state = int(sample["g_fsr_calib_state"])
        freq_hz = sample["g_vibrato_freq_x100"] / 100.0
        amp = int(sample["g_vibrato_amplitude"])
        f1_pct = int(sample["g_fsr1_tension_pct"])
        f1_var = int(sample["g_fsr1_feature_var"])
        f2_pct = int(sample["g_fsr2_tension_pct"])
        
        self.status_label.setText(f"状态标志: {fsr_calib_text(calib_state)}")
        
        if calib_state == 4: # 只有标定就绪后才更新诊断
            self.lbl_freq.setText(f"揉弦频率: {freq_hz:4.2f} Hz")
            self.lbl_amp.setText(f"动作幅度: {amp}")
            self.lbl_f1_info.setText(f"指腹发力: {f1_pct}% (方差: {f1_var})")
            self.lbl_f2_info.setText(f"虎口发力: {f2_pct}%")
            ...
            if freq_hz == 0:
                ...
                self.lbl_ai_vibrato.setStyleSheet("color: #39FF14;") # 荧光绿
            else:
            ...
            if f1_pct > 80 or f2_pct > 80:
                self.lbl_ai_tension.setText("肌肉评估: ❌ 死力警告！手指或虎口过度紧绷！")
                self.lbl_ai_tension.setStyleSheet("color: #FF0000; font-weight: bold;")
            elif f1_var > 5000: # 这里的阈值你可以根据实际测试调整
    ...
    def update_plot(self):
        ...
        # FSR 固定 0 - 100% 高度 (把原始值除以26强行压缩在同一视图里，完美适配 RFP 传感器 2600 的最大值)
        self.fsr1_lpf_curve.setData(self.fsr1_lpf_buffer / 26.0) 
        self.fsr1_tension_curve.setData(self.fsr1_tension_buffer)
        self.fsr1_plot.setYRange(-5, 110, padding=0)

        self.fsr2_lpf_curve.setData(self.fsr2_lpf_buffer / 26.0)
        self.fsr2_tension_curve.setData(self.fsr2_tension_buffer)
        self.fsr2_plot.setYRange(-5, 110, padding=0)
